...
import MapaImgFile from "../imagens/MapaImgFile.jpg";
...
const Content = styled.div`
  flex: 1; /* ocupa todo o espaço disponível, empurrando o footer para o fim */
  display: flex;
...
export default function Mapa() {
  ...
  return (
    <PageWrapper>
      ...
      <Content>
        <TransformWrapper initialScale={1} minScale={0.8} maxScale={3} centerOnInit wheel={{ step: 0.1 }} doubleClick={{ disabled: true }}>
          <TransformComponent>
            <MapContainer>
            <MapaImg src={MapaImgFile} alt="Mapa da fábrica" />


              {locais.map((local) => (
                <Marker key={local.id} style={local.estilo} onClick={() => setLocalSelecionado(local)}>
                  {local.nome}
                </Marker>
              ))}
            </MapContainer>
        ...
        {localSelecionado && (
          <LocationModal local={localSelecionado} onClose={() => setLocalSelecionado(null)} />
        )}
      </Content>
